...
import com.hulk.byod.parser.entity.ActivityLog;
import java.text.DateFormat;
import java.text.SimpleDateFormat;
import java.util.List;

/**
 * 上报至服务端的活动日志保存到txt里面, 这是一个独立的日志文件
 * Created by zhanghao on 2017/11/24.
 */

public class HulkLogManager {

    private final static String TAG = "HulkLogManager";

    //需要上报至服务端的活动日志保存到txt里面
    private final static String LOGS_FILE_PATH = "/logs/report_logs.txt";
    ...
    TxtFile mTxtFile;
    private static HulkLogManager instance;

    public static HulkLogManager getInstance() {
        if (instance == null) {
            synchronized (HulkLogManager.class) {
                if (instance == null) {
                    instance = new HulkLogManager();
                }
            }
        }
        return instance;
    ...
    public static void addActLog(String actName, String act_type, String actDesc) {
        ActivityLog log = HulkLogManager.create(actName, act_type, actDesc);
        boolean added = HulkLogManager.getInstance().addLog(log);
        if (added) {
