...
public class NetPingUtil {
     ...
     * @param remark
     */
    public static void checkInternetConnectedAsync(final Context context, final PingCallback callback, final String remark) {
        if (!NetUtils.isConnected(context)) {
            Log.w(TAG, "checkInternetConnectedAsync ignored for network is disabled ! ");
            //网络没有打开,状态职位false
            setInternetConnectedState(context, false);
            if (callback != null) {
                callback.onPing(false, -1, "network is disabled");
            }
            return;
        }
        new Thread(new Runnable() {
            ...
            public void run() {
                for (String address: TEST_ADDRESSES) {
                    ...
                    if (connected) {
                        Log.w(TAG, "Set InternetConnectedState as true ! ");
                        setInternetConnectedState(context, true);
                        return;
                    }
                }
                Log.w(TAG, "Set InternetConnectedState as false ! ");
                setInternetConnectedState(context, false);
            }
        }, "checkInternetConnectedAsync-Thread").start();
    }

    public static void setInternetConnectedState(Context context, boolean internetConnected) {
    	sInternetConnected = internetConnected;
    }

    public static boolean getInternetConnectedState(Context context) {
