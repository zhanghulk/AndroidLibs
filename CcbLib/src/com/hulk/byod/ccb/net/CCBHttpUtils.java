...
import com.hulk.byod.ccb.CCBEncryptUtils;
...
public class CCBHttpUtils {
    ...
    protected final static String TAG = "CCBHttpUtils";
     ...
     * 远程调试返回本地demo数据
     * @param context
     * @param tranCode one of TS1609031, TS1609050, and...
     * @param postText ccb xml request text data
     * @return
     */
    public static CCBHttpResponse testHttpRequest(Context context, String tranCode, String postText) {
        Log.i(TAG, "testHttpRequest tranCode: " + tranCode + ", postText: " + postText);
        CCBHttpResponse response = new CCBHttpResponse(tranCode);
        response.errorCode = 0;
        response.xmlText = CCBXmlUtils.readResponseDemoXmlText(context, tranCode, false);
     ...
     * @return
     */
    public static CCBHttpResponse startHttpRequest(Context context, String tranCode, String postText) {
        Log.i(TAG, "startHttpRequest tranCode: " + tranCode + ", postText: " + postText);
        CCBHttpResponse response = new CCBHttpResponse(tranCode);
        byte[] encData = CCBEncryptUtils.encryptText(postText);
        if (encData == null) {
        ...
        try {
            ...
            if (!TextUtils.isEmpty(resultEncText)) {
                ...
                if (!TextUtils.isEmpty(decodedText)) {
                    String clearText = CCBEncryptUtils.decryptText(decodedText);
                    response.xmlText = clearText;
                    if (!TextUtils.isEmpty(clearText)) {
                        CCBXmlUtils.writeRespData(tranCode, clearText);
                    } else {
